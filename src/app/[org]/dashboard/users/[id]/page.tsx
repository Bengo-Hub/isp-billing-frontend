  ...
  if (error || !user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <Link href={`/${orgSlug}/dashboard/users`}><Button variant="outline">Back to Users</Button></Link>
      </div>
    );
  }
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div className="flex items-start gap-3">
          <Link href={`/${orgSlug}/dashboard/users`} className="mt-1"><ArrowLeft className="h-5 w-5 text-gray-500 hover:text-gray-700" /></Link>
          <div>
            ...
            {showActions && (
              <>
                ...
                <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-gray-900 rounded-lg shadow-lg border dark:border-gray-700 z-20 py-1">
                  ...
                  <button onClick={() => { setShowActions(false); if (confirm('Delete this user?')) { deleteMutation.mutate(numericId, { onSuccess: () => router.push(`/${orgSlug}/dashboard/users`) }); } }} className="w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 text-sm text-red-600">
