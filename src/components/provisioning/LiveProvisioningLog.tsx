  ...
  useEffect(() => {
    ...
    const connectWebSocket = () => {
      ...
      wsRef.current.onopen = () => {
        setIsConnected(true);
        // WebSocket connected - show status indicator
      };
      ...
      wsRef.current.onmessage = (event) => {
        try {
          ...
          if (data.type === 'log') {
            ...
            // Handle status updates - display in log stream
            const statusLog: LogEntry = {
              timestamp: new Date().toLocaleTimeString(),
              message: `Status: ${data.data.status || 'updated'}`,
              type: 'info'
            };
          ...
          // Silently handle parse errors - non-critical for log display
        }
      };

      wsRef.current.onclose = () => {
        setIsConnected(false);
        // WebSocket disconnected - status indicator shows state
      ...
      wsRef.current.onerror = () => {
        setIsConnected(false);
        // Connection error - status indicator shows disconnected state
      };
    };
